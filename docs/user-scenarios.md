...
> **현재 Windows 8GB NVA 시나리오:** `UC-WIN-NVA-8G`와 [`windows-8gb-nva.md`](windows-8gb-nva.md)가 정본이다. VRAM 8GB 이상, 외부 LLM 유지, 로컬 VoxCPM2 W8A16 + TensorRT LocDiT와 TensorRT-native Ditto, 8GB 미만 NVA disabled가 현재 계약이다. RTX 4060 Laptop/4070 외 모델은 엔진 호환 확인이 필요하다. 아래의 S-8G/UC-AV 3모드와 6GB NVA 내용은 과거 연구 기록이며 Windows 프로파일에 적용하지 않는다.
