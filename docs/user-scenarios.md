...
| **UC-WIN-NVA-8G** (#397, FR-CASCADE.9~14 — 2026-07-31) | Windows의 **NVIDIA RTX VRAM 8GB 이상** PC에서 Naia 계정의 외부 LLM(원격 Ollama 또는 외부 API)을 그대로 사용하면서, 로컬 표현 계층은 VoxCPM2 W8A16 host + FP16 TensorRT LocDiT와 TensorRT-native Ditto로 실행한다. 프로파일은 LLM provider/model/host를 바꾸거나 로컬 Ollama·NPU를 설치/기동하지 않는다. 8GB 미만 또는 VRAM 미확인 상태에서는 NVA 선택·자동기동·재시작 복원을 모두 막고 VRM으로 안전하게 복귀한다. 초기화가 느리거나 실패해도 NVA Player는 idle 화면을 먼저 표시한다. 실시간 속도는 지원 조건이 아니다. | 제어면(온보딩·프로파일·두뇌·아바타) + 표현(NVA Player) + Windows lifecycle | `vram-tiers.test.ts` · `nva-gate.test.ts` · `SettingsTab.test.tsx` · `slots-manifest.contract.test.ts` · `VideoAvatarCanvas.test.tsx` · `capability-settings.spec.ts` · Tauri `94-avatar-4060-facade.spec.ts` · windows-manager `test_profiles_vram_fit.py`/`test_manifest.py` |
| **UC-WIN-VOICE-6G** (#406, FR-CASCADE.20~22 — 2026-08-01) | 등록 후 Naia에 로그인한 사용자는 Windows NVIDIA RTX VRAM 6GB 이상 PC에서 기존 Naia 계정 LLM·원격 Ollama·외부 API 설정을 유지하면서 로컬 VoxCPM2 W8A16 + TensorRT LocDiT 음성을 사용한다. Shell은 3D VRM을 유지하며 Ditto/NVA/로컬 LLM/Ollama/NPU/STT를 시작하지 않는다. 로그아웃 또는 VRAM 미달이면 profile/manifest/IPC가 모두 fail-closed다. 첫 기동이 느려도 기능 대상이지만 실시간 속도와 실제 6GB cold boot는 측정 전 보장하지 않는다. 실제 화면 검증 자산은 `naia-settings/vrm-files/03-OL_Woman.vrm`이며 SHA-256으로 동일성을 고정한다. | 설정 프로파일 + 외부 대화 + 로컬 TTS + VRM + Windows lifecycle | `vram-tiers` · `tier-slots` · `config` · `slots-manifest` · Settings Playwright · Rust account/VRAM/install tests · manager profile/manifest/launch tests · labs CPU-quantization tests · 실제 Tauri Shell voice/VRM probe |
...
| **UC-WIN-VOICE-CONTINUOUS** (FR-VOICE.11 — 2026-08-01) | 여러 문장 답변에서 첫 문장만 들리고 나머지가 `429 busy`로 사라지지 않는다. 첫 문장을 생성한 뒤 다음 문장을 한 개씩 준비하며, 생성이 재생보다 느릴 때는 한 문장을 더 버퍼링해 문장 사이의 무음을 줄인다. | Windows 6GB VoxCPM2 + Shell VRM 연속 발화 | ChatArea/AudioQueue 단위 + 실제 façade 연속 문장 로그에서 200 순서·429 0건 |
