...
| **UC-WIN-NVA-8G** (#397, FR-CASCADE.9~14 — 2026-07-30) | Windows의 **NVIDIA RTX VRAM 8GB 이상** PC에서 Naia 계정의 외부 LLM(원격 Ollama 또는 외부 API)을 그대로 사용하면서, 로컬 표현 계층만 TensorRT VoxCPM2 int8 + Ditto로 실행한다. 프로파일은 LLM provider/model/host를 바꾸거나 로컬 Ollama·NPU를 설치/기동하지 않는다. 8GB 미만 또는 VRAM 미확인 상태에서는 NVA 선택·자동기동·재시작 복원을 모두 막고 VRM으로 안전하게 복귀한다. 로컬 모델 초기화가 느리거나 실패해도 NVA Player는 번들의 idle 화면을 먼저 표시하고 준비/실패 상태 및 재시도를 별도 오버레이로 제공한다. Shell은 “NVA 로컬 아바타는 NVIDIA RTX GPU VRAM 8GB 이상 필요”와 “클라우드 cascade는 향후 별도 제공 예정”을 명시한다. 실시간 속도는 지원 조건이 아니다. | 제어면(온보딩·프로파일·두뇌·아바타) + 표현(NVA Player) + Windows lifecycle | `vram-tiers.test.ts`(8GB 경계·legacy 6GB 차단·외부 LLM) · `nva-gate.test.ts`(로그인 여부와 무관한 <8GB 차단) · `SettingsTab.test.tsx`(provider/model 보존·메뉴 안내·8GB 미만 disabled) · `slots-manifest.contract.test.ts`(TRT loader profile·외부 main 유지) · `VideoAvatarCanvas.test.tsx`(idle 우선·준비/실패/재시도) · `capability-settings.spec.ts`(프로파일/두뇌/아바타 메뉴 교차) · `nva-player-initialization.spec.ts`(기본·진행·성공·오류·좁은 폭) · Tauri `94-avatar-4060-facade.spec.ts`(<8GB spawn 차단, 8GB idle→cascade) · windows-manager `test_profiles_vram_fit.py`/`test_manifest.py`. |
| **UC-WIN-NVA-8G evidence correction** | #397 supersedes the CPU/NPU Ollama brain clauses in S-VOICE-AVATAR and S-8G: the LLM remains external and only TensorRT VoxCPM2 int8 + Ditto run locally. | Same surfaces as UC-WIN-NVA-8G. | Implemented automation: `VideoAvatarCanvas.test.tsx` (idle/start failure/retry/<8GB no-start), `capability-settings.spec.ts`, `settings-slots.spec.ts`, Rust `cascade_vram_requires_detected_nvidia_8gb_or_more`, and windows-manager `test_profiles_vram_fit.py`/`test_manifest.py`. The previously listed `nva-player-initialization.spec.ts` and Tauri 94 VRAM boundary case are planned, not completed evidence. |
