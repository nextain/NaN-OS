...
| S57-AUTH | **ADK 부트스트랩 Naia 로그인**은 데스크톱 OAuth 계약(`redirect=desktop`, `app=naia-os`, loopback `redirect_uri`, CSRF `state`)을 모두 전달하고, 신규 사용자가 약관 동의를 마치면 원래 callback으로 돌아와 크레딧 키를 저장한다. 기존 BYO provider/model은 로그인 완료 뒤 main 역할을 가로채지 못한다. | UC12 | control-plane·config | `adk-login-contract.test.ts` + naia.land consent redirect 계약 |
...
| **S-VOICE-AVATAR** (FR-VOICE.5 — 2026-08-03) | 비디오 아바타가 활성화되면 Shell은 응답 텍스트를 비공개 Naia Media Runtime의 `POST :8910/stream_text`에 한 번만 보낸다. Runtime이 승인된 활성 음성으로 VoxCPM2 합성, Ditto TensorRT NVA 렌더링, 음성·영상 동기화를 수행하고 Shell은 mux된 결과를 음소거하지 않고 재생한다. Shell은 같은 발화를 별도 합성하거나 PCM/WAV를 `/stream`으로 다시 보내지 않는다. 비디오 아바타가 꺼진 음성 전용 경로만 `POST /v1/audio/speech`를 사용한다. Runtime 미가용 또는 승인 음성 부재 시 실패를 드러내며 랜덤·위장 음성을 재생하지 않는다. | 표현(speech+avatar) — CPU/NPU 두뇌와 GPU 표현의 단일 Runtime 결합점 | `media-runtime-routing.contract.test.ts`(아바타 분기 선행·Shell 합성/WAV 재전송 금지) + `cascade-renderer.test.ts`(`/stream_text` 요청·mux 재생) + Runtime `test_openai_speech.py`(승인 기본 음성 자동 활성화) + 실행 중 :8910 계약 probe와 Tauri 재생/speaking/lipsync 관찰 |
| **S-VRAM-AUTO / S-VOICE-AUTO / S-VOICE-PRESET / S-ECHO** (FR-VRAM.5·VOICE.6·VOICE.7·ECHO.1 — 2026-08-03) | 사용자가 **8645H+RTX 4060 8GB** 프로파일을 고르면 두뇌는 CPU/NPU Ollama DNA3(`ollamaNumGpu=0`), 표현은 VRAM VoxCPM2 int8 3.47GB+Ditto 2.6GB(계획 6.07GB)로 자동 설정되고 half-duplex로 동작한다. 16GB Windows 프로파일은 `local_16g`로 고정되어 로컬 LLM+VoxCPM2 int8+Ditto TensorRT(12.37GB)를 올리고 NVA 번들을 Ditto가 처리한다. Shell은 필요한 로컬 모델·런타임을 다운로드 중/준비 중/실행 중으로 일관되게 보여 준다. 두 경우 모두 음성·아바타는 로컬 :8910 Runtime을 사용하며, 승인된 원격 사설 Runtime 호스트는 보존한다. 프리셋 음색은 Runtime 팔레트 id로 전달된다. 음성 대화 중 나이아 자기 목소리가 마이크로 되들어와도 (1)재생 중 마이크 정지 (2)직전 발화 유사도 스킵으로 사용자 입력에 섞이지 않으며, **"좋아/네" 같은 짧은 정상 답변은 삼켜지지 않는다** | 제어면(프로필·다운로드/준비 상태) + 표현(음색·에코·NVA) | `vram-tiers.test.ts`·`SettingsTab.test.tsx`·`slots-manifest.contract.test.ts`·`synthesize.test.ts`·`echo-text-filter.test.ts`·loader `plan --profile laptop_4060_8g`/`local_16g`·Tauri `settings-slots` 및 다운로드/준비 상태 UC. 실제 음색/립싱크는 실행 중 :8910 의미 계약과 Tauri 관찰을 함께 통과해야 한다. |
...
| **S-CASCADE** (FR-CASCADE — 2026-08-03 개정) | 설정에서 로컬 미디어 기능을 시작하면 Windows Manager가 비공개 Naia Media Runtime의 설치·업데이트·GPU 확인·실행·중단·복구만 담당한다. Shell은 준비 상태와 capability를 표시하고 `:8910` 단일 의미 계약만 소비한다. 독립 Cascade 제품이나 원시 VoxCPM2·Ditto 포트는 사용자 설정·소비자 계약으로 노출하지 않는다. | 제어면/표현 — Runtime 생명주기와 소비 계약 분리 | windows-manager lifecycle 테스트 · Shell capability/health 계약 · :8910 `/health`·`/v1/audio/speech`·`/stream_text` 통합 probe |
...
| **S-NVA-IDLE** (#394) | 선택한 NVA 번들의 로컬 idle 클립은 음성·cascade 준비 여부와 무관하게 즉시 반복 재생된다. cascade가 준비되면 말하기 영상만 위에 연결하며, 장애가 나도 idle은 유지된다. | 표시계면(NVA idle/발화) | `VideoAvatarCanvas.test.tsx` |
| **S-MARKDOWN-OPEN** (#190) | OS에서 `naia file.md` 또는 Markdown 파일 연결로 앱을 열면 첫 실행과 이미 실행 중인 단일 인스턴스 모두 해당 문서를 Workspace 편집기/미리보기로 연다. | 표시계면 + OS 통합 | Rust argv resolver 계약 + WorkspaceCenterArea 이벤트 계약 |
| **S-MARKDOWN-PRINT** (#395) | Markdown 미리보기를 인쇄할 때 여러 페이지 문서, 표, 긴 코드, Mermaid가 잘리거나 UI chrome과 함께 출력되지 않는다. | 표시계면 | print media Playwright 계약 |
...
> - **UC-AV.8 NVA 결합 발화**: 로컬 또는 승인된 원격 Naia Media Runtime에 응답 텍스트를 `/stream_text`로 보내고 Runtime이 mux한 VoxCPM2 음성+Ditto TensorRT NVA 영상을 그대로 재생한다. 이 경로에서는 Shell의 별도 음성 합성이나 PCM/WAV 재전송을 하지 않는다. 배경 투명은 Runtime의 VP9 알파 출력이 활성일 때만 제공한다.
