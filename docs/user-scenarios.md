...
| **UC-NVA-OFFLINE** (FR-NVA.1~4 — 2026-08-05) | 사용자는 로그인·GPU·로컬 음성 프로필 없이 온보딩 또는 설정에서 VRM이나 여섯 NVA(지나·민호·나이아의 기본/애니 화풍)를 선택한다. NVA는 idle을 즉시 재생하고 기존 TTS가 말하는 동안 번들의 muted talking 클립으로 근사 표현한 뒤 idle로 복귀한다. 원격 또는 로그인된 로컬 정밀 미디어 런타임이 준비되면 같은 선택을 유지한 채 정밀 스트림으로 승격하고, 실패하면 저장 클립 재생으로 안전하게 폴백한다. | 온보딩·설정 + GPU 없는 NVA Player + 선택적 정밀 Runtime | `nva-gate.test.ts` · `nva.test.ts` · `VideoAvatarCanvas.test.tsx` · `OnboardingWizard.test.tsx` · Settings/Onboarding Playwright |
| **UC-WIN-NVA-8G** (#397·#413, FR-CASCADE.9~14 — 2026-08-02, 2026-08-05 게이트 분리) | Windows의 **지원되는 NVIDIA GPU, VRAM 8GB 이상** PC에서 Naia 계정의 외부 LLM(원격 Ollama 또는 외부 API)을 그대로 사용하면서 로컬 음성과 정밀 NVA 립싱크 Runtime을 실행한다. 프로파일 UI는 구현명을 노출하지 않으며 LLM provider/model/host를 바꾸거나 로컬 Ollama·NPU를 설치·기동하지 않는다. 보안 저장소의 로그인은 재시작 뒤 회원 manifest에 복원된다. 8GB 미만·VRAM 미확인·로그아웃 상태는 정밀 Runtime 자동기동만 막고, NVA 선택과 저장 idle/talking 클립 재생은 UC-NVA-OFFLINE에 따라 유지한다. 초기화가 느리거나 실패해도 NVA Player는 idle 화면을 먼저 표시한다. 실시간 속도는 지원 조건이 아니다. | 제어면(온보딩·프로파일·두뇌·아바타) + 표현(NVA Player) + Windows lifecycle | secure-store App 회귀 · `adk-store.test.ts` manifest 재덮기 회귀 · `VideoAvatarCanvas.test.tsx` · `vram-tiers.test.ts` · `nva-gate.test.ts` · `SettingsTab.test.tsx` · 실제 `tauri:dev` manifest/8910 health/NVA 캡처 |
