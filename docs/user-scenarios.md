...
> **현재 Windows 8GB NVA 시나리오:** `UC-WIN-NVA-8G`와 [`windows-8gb-nva.md`](windows-8gb-nva.md)가 정본이다. VRAM 8GB 이상, 외부 LLM 유지, 로컬 VoxCPM2 CUDA/PyTorch INT8 + TensorRT-native Ditto, 8GB 미만 NVA disabled가 현재 계약이다. RTX 4060 Laptop/4070 외 모델은 엔진 호환 확인이 필요하다. 아래의 S-8G/UC-AV 3모드와 6GB NVA 내용은 과거 연구 기록이며 Windows 프로파일에 적용하지 않는다.
...
| **S-VOICE-AVATAR** (FR-VOICE.5 — #397 갱신) | Windows 8GB 프로파일은 LLM을 외부(Naia 계정·원격 Ollama·외부 API)에 유지한다. 로컬 표현은 VoxCPM2 CUDA/PyTorch INT8 음성을 재생하고 **같은 WAV를 Ditto TensorRT-native `/stream`에 직렬 전달**해 립싱크한다. `windows_trt_8g`의 이름은 호환용이며 VoxCPM2 TensorRT 완료를 뜻하지 않는다. 음성 서버 미가용이면 NVA idle을 유지하고 1회 알림+무음 원칙을 따른다. | 표현(speech+avatar) — 외부 두뇌와 GPU 표현의 half-duplex 결합점 | `synthesize.test.ts` · `cascade-renderer.test.ts` · 실제 :8910→:8901/:8902 façade probe · Tauri 94 NVA 출력/발화 |
| **UC-WIN-NVA-8G** (#397, FR-CASCADE.9~14 — 2026-07-30) | Windows의 **NVIDIA RTX VRAM 8GB 이상** PC에서 Naia 계정의 외부 LLM(원격 Ollama 또는 외부 API)을 그대로 사용하면서, 로컬 표현 계층은 VoxCPM2 CUDA/PyTorch INT8 + TensorRT-native Ditto로 실행한다. 프로파일 이름의 `trt`는 Ditto 경로를 가리키며 VoxCPM2 TensorRT는 아직 미구현이다. 프로파일은 LLM provider/model/host를 바꾸거나 로컬 Ollama·NPU를 설치/기동하지 않는다. 8GB 미만 또는 VRAM 미확인 상태에서는 NVA 선택·자동기동·재시작 복원을 모두 막고 VRM으로 안전하게 복귀한다. 초기화가 느리거나 실패해도 NVA Player는 idle 화면을 먼저 표시한다. 실시간 속도는 지원 조건이 아니다. | 제어면(온보딩·프로파일·두뇌·아바타) + 표현(NVA Player) + Windows lifecycle | `vram-tiers.test.ts` · `nva-gate.test.ts` · `SettingsTab.test.tsx` · `slots-manifest.contract.test.ts` · `VideoAvatarCanvas.test.tsx` · `capability-settings.spec.ts` · Tauri `94-avatar-4060-facade.spec.ts` · windows-manager `test_profiles_vram_fit.py`/`test_manifest.py` |
| **UC-WIN-NVA-8G evidence correction** | #397 supersedes the CPU/NPU Ollama brain clauses in S-VOICE-AVATAR and S-8G: the LLM remains external and only VoxCPM2 CUDA/PyTorch INT8 + TensorRT-native Ditto run locally. | Same surfaces as UC-WIN-NVA-8G. | Implemented automation: `VideoAvatarCanvas.test.tsx` (idle/start failure/retry/<8GB no-start), `capability-settings.spec.ts`, `settings-slots.spec.ts`, Rust `cascade_vram_requires_detected_nvidia_8gb_or_more`, windows-manager profile/manifest tests, and Tauri 94 real 4060 output path. VoxCPM2 TensorRT backend and model-wide RTX compatibility gates remain follow-up work. |
...
| **S-CASCADE-HISTORICAL** (Round 2, 2026-06-30) | 과거 8GB 음성 단독/fp16 가정 기록. **현재 Windows 8GB 실행 계약이 아니다.** 현재는 `windows_trt_8g`가 VoxCPM2 INT8 + Ditto를 함께 계획한다. | 이력 | 현재 시나리오는 UC-WIN-NVA-8G 참조 |
...
> - **UC-AV.7 voiceprint 불변(NFR)**: Naia가 VoxCPM2를 사용할 때 **음성지문(ref)은 필수**이며 무지문 합성을 허용하지 않는다. 이 원칙은 Windows 8GB 로컬 VoxCPM2 INT8에도 적용된다. 검증: voiceprint guard와 façade `/ref/voices` 팔레트.
