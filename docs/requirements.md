...
| **FR-V017.12** | The Windows bundle contains a pinned `uv` bootstrap, the exact cascade façade/voice service/reference assets, and an installer manifest. `install_cascade_runtime` is single-flight, writes a durable log, creates the managed Python 3.10 voice environment, installs pinned CUDA/voice dependencies, downloads the pinned VoxCPM2 revision, builds and verifies a device-specific TensorRT engine, and returns refreshed step status. Status probes have no install side effects; install failure is retryable and cannot select or start local voice. | staging source/hash/manifest tests + Rust command/process/classification tests + Settings/onboarding progress/error/retry + clean RTX Windows install/start acceptance |
...
| **FR-V017.19** (#450, supersedes the disabled-selector behavior in FR-V017.11 and the generic-loader packaging clause in FR-V017.12 for Windows voice-only) | On Windows NVIDIA hardware meeting the 6 GB gate, selecting local VoxCPM2 while uninstalled starts a transactional `windows_trt_6g` provision flow. Shell installs pinned Python/packages/model assets, builds and verifies TensorRT LocDiT, launches the direct VoxCPM2 voice service, waits for health, then atomically persists `naia-local-voice`. The selector remains actionable while uninstalled; only ineligible hardware is disabled. Download/build/start/health failure rolls every provider/enable/slot value back, retains phase-specific stderr and a safe log path, and permits retry. User-facing status and prerequisite names must describe VoxCPM2/TensorRT, not a generic cascade loader or service bundle. | Rust install/direct-launch/status tests + Settings/onboarding transactional selector tests + clean Windows native install/start acceptance; RTX 2070 is measurement-gated |
