...
> **#397 supersession:** FR-CASCADE.9~14 replace the CPU/NPU/local-Ollama brain clauses in FR-VOICE.5 and FR-VRAM.5. The active Windows profile keeps the LLM external and runs only TensorRT VoxCPM2 int8 + Ditto locally.
...
| **FR-CASCADE.9** (#397, supersedes the Windows 8GB parts of FR-VRAM.5 and the 8GB focus assumptions above) | Windows local profile `laptop-4060-8g` is compatibility UI identity only. Its canonical loader profile is `windows_trt_8g`, and its only local capabilities are TensorRT VoxCPM2 int8 TTS + TensorRT Ditto avatar (planned footprint 6.07GB). It never selects, installs, or launches a local LLM, Ollama, or NPU runtime. Real-time performance is not a support gate. | UC-WIN-NVA-8G | `vram-tiers.test.ts`, slots manifest contract, windows-manager profile/manifest tests |
...
| **FR-CASCADE.11** | NVA local avatar requires a detected NVIDIA RTX GPU with **VRAM 8GB or more**. A detected value below 8GB or an unknown value disables the NVA option, prevents legacy 6GB/8GB override activation, prevents manifest loader-profile emission and `start_cascade`, and renders VRM instead of a stale NVA config. | UC-WIN-NVA-8G | `vram-tiers.test.ts`, `nva-gate.test.ts`, `capability-settings.spec.ts`, Rust start guard/Tauri test |
...
| **FR-CASCADE.13** | Profile and Avatar menus display: “NVA local avatar requires NVIDIA RTX GPU VRAM 8GB or more.” The copy must not imply RTX 50 support under the current TensorRT 8.6 package; current Windows package support is RTX 30/40 8GB+ and RTX 50 remains a TensorRT 10.8+ follow-up. | UC-WIN-NVA-8G | i18n key coverage, SettingsTab unit + Playwright text assertions |
