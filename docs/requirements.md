...
| **FR-ENV-ATTENTION.15** | 줄어든 양을 직접 잰다. 고정 표본(표면 12개) 기준 요청당 1187→77바이트(93.5% 감소), 40턴 총량은 최악의 경우(켜 놓고 잊음)에도 47480→13070바이트(72.5% 감소). 계약 테스트가 이 수치를 ±8% 안에서 위아래로 묶는다 — 위는 부풀기를, 아래는 세그먼트를 통째로 없애 수치를 좋게 만드는 것을 막는다. 같은 절감을 **실제로 나간 chat_request** 에서도 잰다. | UC-ENV-ATTENTION | `environment-live-wiring.contract.test.ts`, `environment-skill.spec.ts` | Done |
