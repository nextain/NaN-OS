...
| **FR-CASCADE.11** | The precision local media runtime requires a detected, supported NVIDIA GPU with **VRAM 8GB or more** plus an authenticated explicit local profile. A detected value below 8GB or an unknown value prevents manifest loader-profile emission and `start_cascade`, but does not disable NVA bundle selection, idle playback, or stored talking-clip fallback. | UC-WIN-NVA-8G·UC-NVA-OFFLINE | `vram-tiers.test.ts`, `nva-gate.test.ts`, `VideoAvatarCanvas.test.tsx`, Rust start guard/Tauri test |
...
| **FR-NVA.1** | `avatarProvider=naia-video-avatar` 선택과 복원은 로그인, 감지 GPU, VRAM tier, VoxCPM2 또는 로컬 프로필의 존재와 독립적이다. 이 조건들은 정밀 로컬 Runtime 자동기동만 통제한다. | UC-NVA-OFFLINE | `nva-gate.test.ts`, SettingsTab 단위/Playwright |
| **FR-NVA.2** | Shell은 선택한 NVA manifest에서 idle과 talking 클립을 로컬로 읽는다. 정밀 Runtime이 없을 때 기존 TTS 재생 상태와 함께 muted talking 클립을 반복하고 발화 종료·중단·오류 뒤 idle로 복귀한다. 번들 내장 음성은 기존 TTS와 중복 재생하지 않는다. | UC-NVA-OFFLINE | `nva.test.ts`, `VideoAvatarCanvas.test.tsx`, ChatArea speaking-state 회귀 |
| **FR-NVA.3** | 원격 Runtime 또는 인증된 로컬 8GB+ 프로필이 준비되면 정밀 스트림을 우선한다. 연결·기동 실패, 로그아웃, VRAM 미달은 선택을 VRM으로 강등하지 않고 저장 클립 fallback을 유지한다. | UC-NVA-OFFLINE·UC-WIN-NVA-8G | `VideoAvatarCanvas.test.tsx`, `nva-remote-idle.live.spec.ts` |
| **FR-NVA.4** | 온보딩의 캐릭터 단계와 설정은 설치된 VRM 및 NVA를 함께 노출하고, NVA 선택 시 `avatarProvider`와 bare `nvaModel`을 저장한다. 기본 자산은 지나·민호·나이아의 기본/애니 화풍 6종이며 선택은 재시작 뒤 보존된다. | UC-NVA-OFFLINE·UC12 | `OnboardingWizard.test.tsx`, `onboarding-fresh.spec.ts`, SettingsTab 단위/Playwright |
