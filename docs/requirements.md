...
| **FR-ENV-ATTENTION.15** | 줄어든 양을 직접 잰다. 표면 12개 기준 요청당 1187→77바이트(93.5% 감소), 40턴 총량은 최악의 경우(켜 놓고 잊음)에도 47480→13070바이트(72.5% 감소). 회귀하면 계약 테스트가 깨진다. | UC-ENV-ATTENTION | `environment-live-wiring.contract.test.ts` | Done |
