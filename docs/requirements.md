...
| **FR-ADK-AUTH.1** | ADK 부트스트랩 로그인 URL은 `redirect=desktop`, `app=naia-os`, 허용된 loopback `redirect_uri`, 64자리 hex `state`를 함께 보내며 신규 사용자 동의 뒤에도 callback 문맥을 보존한다. | S57-AUTH | `adk-login-contract.test.ts` + naia.land consent 계약 |
| **FR-ADK-AUTH.2** | 로그인 완료 저장은 기존 설정을 보존하되 main provider/role과 legacy mirror를 `nextain` 및 유효한 gateway 모델로 권위 있게 덮어써 크레딧 우회를 막는다. | S57-AUTH | `adk-login-contract.test.ts` |
...
| **FR-VOICE.5** | **Naia Media Runtime 단일 음성+비디오 아바타 경로**. ① 비디오 아바타 활성 시 Shell은 텍스트를 `POST :8910/stream_text`에 한 번만 보내고 Runtime이 VoxCPM2 합성, Ditto TensorRT NVA 렌더링, 음성·영상 동기화를 소유한다. ② Shell은 같은 발화에 `synthesizeTts`, AudioQueue 음성 재생, `speakAudio`, PCM/WAV→`/stream` 조합을 사용하지 않으며 mux 응답의 음성을 재생한다. ③ `/stream`은 이미 생성된 외부 오디오를 가진 승인 소비자용 제한 경로로만 남긴다. ④ Runtime은 승인된 기본 음성 파일이 있으면 시작 시 활성화하고, 활성 음성이 없으면 랜덤 음성으로 폴백하지 않는다. ⑤ 음성 전용 경로는 FR-VOICE.6의 `/v1/audio/speech`를 유지한다. | S-VOICE-AVATAR | `media-runtime-routing.contract.test.ts` + `cascade-renderer.test.ts`(`/stream_text` 요청) + Runtime `test_openai_speech.py`(기본 음성) + 실행 중 :8910 계약 probe + Tauri UI 관찰 |
...
> (9B 로컬 LLM + VoxCPM2 int8 로컬 음성 + Ditto TensorRT NVA). FR-VRAM.4("추천만, 자동변경·숨김 없음")를
> **본 FR 이 개정**한다 — 루크가 명시적으로 자동 적용을 요구했으므로 추천→자동적용으로 전환.

| ID | 요구사항 | UC/시나리오 | 검증(P02) |
|----|----------|-----------|------|
| **FR-VRAM.5** | **검증-티어 전용 프로파일 + 자동설정** (FR-VRAM.4 개정). ① `hidden` 티어는 피커 비노출 **+ `selectVramTier`(auto) 제외** — 미검증 티어 자동선택 차단. 검증 티어에는 `laptop-4060-8g`와 `local-llm-voice-16g`가 포함된다. 4060 8GB는 DNA3를 CPU/NPU(`ollamaNumGpu=0`)에 두고 VRAM에는 VoxCPM2 int8 3.47GB+Ditto 2.6GB(계획 6.07GB)를 half-duplex로 둔다. 16GB는 Windows manager `local_16g`(`llm_main`+`tts_voxcpm2_int8`+`avatar_ditto_trt`, 12.37GB)로 고정한다. ② 프로파일 선택 = **자동설정**(stageLocalSlots): 두뇌(로컬 LLM capability → provider=ollama + compact DNA3), 음성(tts capability → naia-local-voice + :8910 façade), 아바타(avatar capability → naia-video-avatar + NVA 기본 번들). NVA 립싱크 추론은 Ditto TensorRT가 처리한다. ③ slots manifest의 `gpu.loaderProfile`로 Windows manager의 이름 프로필을 직접 지정하며 저장값·구 id 하위호환(normalizeTierId)을 유지한다. | S-VRAM-AUTO | `vram-tiers.test.ts`(4060/16G capability·loaderProfile·hidden auto 제외) · `SettingsTab.test.tsx`(4060/16G 프로파일 자동설정) · `slots-manifest.contract.test.ts` · loader `plan --profile laptop_4060_8g`/`local_16g` · Tauri 다운로드/준비 상태 UI |
| **FR-VOICE.6** | **로컬 음성 정본 호스트 = 비공개 :8910 Runtime**. `DEFAULT_LOCAL_VOICE_HOST` = `http://localhost:8910`이며 음성 전용 표면은 `POST /v1/audio/speech`, 음성+비디오 아바타 표면은 `POST /stream_text`다. raw `/tts`와 VoxCPM2·Ditto 원시 포트를 Shell이 직접 호출하지 않는다. 프로파일 자동설정은 **빈 값·localhost/127.0.0.1 변형만** 이 기본으로 교체하고 승인된 원격 사설 Runtime 호스트는 보존한다. | S-VOICE-AUTO·S-VOICE-AVATAR | `synthesize.test.ts`(:8910 `/v1/audio/speech` 기본 호스트) · `media-runtime-routing.contract.test.ts` · `SettingsTab` 자동설정(원격 호스트 보존) |
| **FR-VOICE.7** | **프리셋 음색 Runtime 팔레트 id 전달**. naia-local-voice 의 `voice` = 사용자 음성 참조(`voiceRefUrl`)의 basename(쿼리/프래그먼트 제거 후 `.wav` 파일명 → Runtime `/ref/voices` 팔레트 id). 팔레트 밖 값(녹음/업로드·비-wav)은 `naia-default`로 정규화하고 Runtime도 미등록 id·참조파일 부재를 400으로 거부한다. **vllm provider 는 제외** — 범용 OpenAI 서버라 팔레트 id 를 모름, `"default"` 유지. 두 음성 전용 합성 경로(파이프라인·Live)가 단일 `resolveTtsVoiceId(config)`를 공유한다. 비디오 아바타 결합 발화의 활성 음성은 Runtime `/voice` 계약이 소유한다. | S-VOICE-PRESET·S-VOICE-AVATAR | `ChatArea` 음색 해석(프리셋→id·쿼리스트링·vllm 분리) · Runtime `test_openai_speech.py`(미등록/파일부재 400) |
...
| **FR-CASCADE.8** | **로컬·승인 원격 NVA 결합 발화·투명성 계약** — Runtime 위치와 무관하게 Shell은 `/stream_text`에서 서버가 합성한 음성과 아바타 영상을 함께 받아 중복 음성을 재생하지 않는다. 같은 텍스트를 Shell이 먼저 합성해 `/stream` 영상과 분리 재생하는 경로는 금지한다. 투명 배경은 Runtime이 VP9 `yuva420p` 알파 영상을 제공할 때만 성립하며, 불투명 H.264를 클라이언트에서 투명하다고 위장하지 않는다. | S-CASCADE-T3·S-VOICE-AVATAR·UC-AV | `media-runtime-routing.contract.test.ts`·`cascade-renderer.test.ts`(muxed `/stream_text`)·`nva-remote-idle.live.spec.ts` |
| **FR-CASCADE.9** | 선택한 NVA 번들의 local idle loop는 cascade/음성 health와 독립적으로 로드·재생한다. cascade는 준비된 뒤 발화 레이어만 담당하며 실패 시 idle을 제거하지 않는다. | S-NVA-IDLE·UC-AV | `VideoAvatarCanvas.test.tsx` |

| **FR-WORKSPACE-MD.1** | `.md`/`.mdx` argv를 절대·현재 작업 디렉터리 기준으로 안전하게 해석하고, 첫 실행 및 single-instance 재호출 모두 `file_open` 계약으로 Workspace에 전달한다. 번들 파일 연결을 Editor로 선언한다. | S-MARKDOWN-OPEN | Rust resolver tests + Workspace event test |
| **FR-WORKSPACE-PRINT.1** | Markdown 인쇄는 앱 chrome을 숨기고 인쇄 여백을 페이지 규칙으로 적용하며 표·긴 코드·Mermaid를 페이지 폭 안에서 읽을 수 있게 배치한다. | S-MARKDOWN-PRINT | Playwright print media test |
