...
| **FR-ENV-ATTENTION.16** | 환경 도구는 대화 턴마다 다시 등록한다. 부팅 등록은 agent 기동과 경쟁하고 agent 가 재시작하면 조용히 사라진다. 등록 전달이 실패하면 그 턴에는 표면을 싣지 않는다 — 없는 도구를 부르라고 안내하지 않는다. 등록 실패가 대화를 막지는 않는다. 설정 전환의 등록·해제도 전달 여부를 확인한다. | UC-ENV-ATTENTION | `environment-skill.spec.ts` | Done |
| **FR-ENV-ATTENTION.15** | 줄어든 양을 직접 잰다. 고정 표본(표면 12개) 기준 요청당 1187→77바이트(93.5% 감소), 40턴 총량은 최악의 경우(켜 놓고 잊음)에도 47480→13070바이트(72.5% 감소). 계약 테스트가 **네 수치 모두**를 ±8% 안에서 위아래로 묶는다 — 위는 부풀기를, 아래는 세그먼트를 없애거나 예산을 줄여 수치만 좋게 만드는 것을 막는다. 감소율 자체도 따로 확인해, 두 값이 같이 움직여 비율만 유지되는 경우를 막는다. 같은 절감을 **실제로 나간 chat_request** 에서도 잰다. | UC-ENV-ATTENTION | `environment-live-wiring.contract.test.ts`, `environment-skill.spec.ts` | Done |
