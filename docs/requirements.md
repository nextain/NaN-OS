...
| **FR-V017.34** (#453) | VoxCPM2 release staging, completed-ZIP audit, and installed-payload activation consume one tracked activation contract covering every archive-owned required file/directory, every installer-created runtime directory, and the exact compiled entry module. A release build fails before Tauri packaging if either the source artifact or selected completed ZIP cannot satisfy the archive-owned contract. The installer materializes declared empty runtime-state directories before activation, so ZIP formats that omit empty directories cannot create a false provenance failure. Runtime rejection lists every failed predicate instead of returning an opaque provenance error. File-inventory manifests continue to exclude their own manifest file, while archive inventory counts the manifest itself; that intentional difference is not an activation failure. | activation-contract fixture mutations + selected real ZIP audit + Rust directory-materialization/multi-failure diagnostics + final release build preflight |
