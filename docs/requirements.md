...
> **현재 Windows 8GB NVA 기준:** [`windows-8gb-nva.md`](windows-8gb-nva.md)와 FR-CASCADE.9~14가 정본이다. VRAM 8GB 이상에서 LLM은 외부(Naia 계정·원격 Ollama·외부 API)로 유지하고, 로컬에는 VoxCPM2 CUDA/PyTorch INT8 + TensorRT-native Ditto만 실행한다. 8GB 미만·VRAM 미감지는 NVA disabled다. RTX 4060 Laptop/4070 실측 외 RTX 30/40은 목표 지원 범위다. 아래의 6GB NVA, 8GB 3모드, CPU/NPU Ollama 내용은 과거 연구 기록이며 이 프로파일에 적용하지 않는다.
...
> **#397 supersession:** FR-CASCADE.9~14와 [`windows-8gb-nva.md`](windows-8gb-nva.md)가 Windows 8GB 정책의 정본이다. FR-VOICE.5와 과거 FR-VRAM.5의 CPU/NPU/local-Ollama 두뇌 조항은 폐기됐다. 활성 Windows 프로파일은 LLM을 외부에 유지하고 VoxCPM2 CUDA/PyTorch INT8 + TensorRT-native Ditto만 로컬에서 실행한다. `windows_trt_8g`는 호환 이름이며 VoxCPM2 TensorRT 구현 완료를 뜻하지 않는다.
...
| **FR-VOICE.5** | **Windows 8GB half-duplex 로컬 음성 + Ditto 립싱크 배선**. ① LLM은 Naia 계정·원격 Ollama·외부 API 경로를 그대로 유지하며 Shell/manager가 로컬 Ollama나 NPU를 선택·설치·기동하지 않는다. ② `naia-local-voice`는 public cascade façade를 통해 VoxCPM2 CUDA/PyTorch INT8 음성을 합성한다. ③ Shell은 재생하는 동일 WAV를 Ditto TensorRT-native `/stream`에 직렬 전달한다. ④ 초기화 실패 시 NVA idle 화면을 유지하고 FR-VOICE.2의 명확한 알림+무음 원칙을 따른다. VoxCPM2 TensorRT는 아직 구현되지 않았다. | S-VOICE-AVATAR·UC-WIN-NVA-8G | `synthesize.test.ts` + `cascade-renderer.test.ts` + 실제 façade `/v1/audio/speech`·Ditto `/stream` + Tauri 94 |
...
|----|----------|-----------|------|
| **FR-VRAM-LEGACY.1** (비규범 이력) | 과거 4060 CPU/NPU Ollama 자동설정 설계. #397에서 폐기됐다. 현재 8GB Windows 프로파일은 `windows_trt_8g`로 정규화하고 LLM 설정을 보존하며 VoxCPM2 CUDA/PyTorch INT8 + Ditto TensorRT-native만 로컬에 둔다. 16GB 로컬 LLM 실험 프로파일은 Windows 8GB 지원 계약과 별도다. | 역사 기록 | 현재 요구사항은 FR-CASCADE.9~14 참조 |
...
> NFR: F1(RTF measurement-gated — VRAM 적합≠실시간 보장). 과거 “8GB 음성 단독” DEFER는 #397로 폐기됐다. 현재 8GB 경로는 VoxCPM2 INT8 + Ditto를 half-duplex로 실행한다. Windows 강제종료 고아 하드닝(job object/PID stale-kill)은 후속이다.
...
| **FR-CASCADE.9** (#397, supersedes the Windows 8GB parts of FR-VRAM.5 and the 8GB focus assumptions above) | Windows local profile `laptop-4060-8g` is compatibility UI identity only. Its canonical loader profile name is `windows_trt_8g`, and its only local capabilities are VoxCPM2 CUDA/PyTorch weight-only INT8 TTS + TensorRT-native Ditto avatar (planned footprint 6.07GB). It never selects, installs, or launches a local LLM, Ollama, or NPU runtime. Real-time performance is not a support gate. VoxCPM2 TensorRT is an unmet follow-up, not a completed capability. | UC-WIN-NVA-8G | `vram-tiers.test.ts`, slots manifest contract, windows-manager profile/manifest tests |
...
| **FR-CASCADE.13** | Profile and Avatar menus display: “NVA local avatar requires NVIDIA RTX GPU VRAM 8GB or more.” This is a minimum VRAM gate, not a guarantee for every RTX model. Verified hardware is RTX 4060 Laptop and RTX 4070; RTX 30/40 is the target range pending model-specific Ditto engine compatibility, and RTX 50 is unverified. | UC-WIN-NVA-8G | i18n key coverage, SettingsTab unit + Playwright text assertions |
...
> NFR: **NFR-voiceprint(불변)** — Naia가 VoxCPM2를 쓸 때 **음성지문(ref voiceprint)은 필수**이며 무지문 합성을 허용하지 않는다. 이 원칙은 Windows 8GB 로컬 VoxCPM2 INT8에도 적용된다. · NFR-honesty(백엔드·VRAM 강등 위장 금지) · F1(RTF 단정 금지). ⚠️ in-shell WSL cascade 부트스트랩은 구 gateway-in-WSL 아키텍처의 레거시다.
