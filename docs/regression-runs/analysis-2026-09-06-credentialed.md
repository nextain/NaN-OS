...
## G 후속 — 시딩은 여덟 중 하나만 설명했다 (#568, 실측으로 갈림)

시딩에서 최상위 `provider`/`model` 거울과 `onboardingComplete`, 화면 기본값을 모두
빼고 `llmRoles.main` 하나만 남긴 뒤 세 번 돌렸다.

| 스펙 | 전 | 후 |
|---|---|---|
| `81-chat-tts-response` | `ttsProvider` = `nextain` | `null` — 게이트웨이 공급자로 고정되던 것이 풀렸다 |
| `03-basic-chat` | (등급 실행에서 flaky) | **EXIT=0** — 게이트웨이로 실제 대화 |
| `56-settings-voice` | 2 통과 / 4 실패 | 2 통과 / 4 실패 — **그대로** |
| `19-skills-bulk` | 2 통과 / 1 실패 | 2 통과 / 1 실패 — **그대로** |

그래서 G 는 하나가 아니라 둘이었다. 시딩이 설명하는 것은 화면 공급자가 바뀌던
자리(81)뿐이고, 설정 섹션과 목록이 비는 것(19·54b·56·71·73·77·79)은 **다른 원인**이다.
`#tts-toggle`·`#tts-provider-select`·`.skill-card` 는 지금도 화면 소스에 있으므로
요소가 없어진 것은 아니다. 반면 `stt-provider-select` 는 소스에 없다 — 77 은 셀렉터
드리프트다. 설정 화면이 세 탭으로 개편된 뒤 e2e 셀렉터가 전면 드리프트라는 기록이
이미 저장소에 있다(70c 주석, #476 후속). 그쪽으로 따로 추적해야 한다.

두 주장을 실측으로 갈랐다는 것도 남긴다. "`onboardingComplete` 를 빼면 시딩이
사라진다" 는 틀렸고("하이드레이션이 파일에서 그 값을 지운다"), "`llmRoles.main` 이
우선하므로 거울은 없어도 된다" 는 맞았다 — 한 세션을 돌린 뒤 그 워크스페이스의
`config.json` 에 `provider: "gemini"`(화면이 되쓴 값)와
