...
## B8 — field-review fixes (2026-08-08)

A live-field review the day after B1~B6 (`docs/voice-avatar-radio-handoff-2026-08-07.md`) found defects that survived the "done" B1~B6 evidence above, because that evidence covered lifecycle/toggle plumbing but not the actual speech-ownership and render path. Items 1~6 were the first pass; the user then directly challenged the first pass's radio-DJ and VoxCPM2-onboarding claims and pushed a second pass (items 7~9) that found a real bug the first pass missed by pattern-matching the handoff doc's own speculative wording instead of reading the code.

1. **TTS single ownership (WNV-06/WNV-07 completion)**: `ChatArea.sendSentenceToTts` unconditionally called `cascadeAvatar.speak(text, undefined, …)` for every NVA sentence, bypassing `synthesizeTts` entirely and falling through to `PrebakedAvatarRenderer`'s internal `window.speechSynthesis` fallback — so no matter which TTS provider (VoxCPM2, cloud) the user selected, only free browser speech ever played. Fixed by splitting the renderer contract into `hasAuthoredClip`/`playAuthoredClip` (exact-phrase clips that carry their own recorded voice — the one legitimate audio-owning case) and `setSpeakingVisual` (idle/talking visual only, driven by Shell's real `AudioQueue`/`speechSynthesis` playback lifecycle). `PrebakedAvatarRenderer.speak/speakAudio/browserSpeech` are removed outright.
2. **NVA slot status accuracy**: `SettingsTab.slotLiveStatus("avatar")` read the retired cascade façade's `avatar_enabled`/`avatar` health fields, which no backend ever sets for a prebaked NVA player — so the row was permanently stuck at "starting" even while playing correctly. Replaced with the real `useCascadeAvatarStore` renderer/`nvaLoadError` state (running / starting / error).
3. **NVA black background during speech**: the shipped `naia` bundle's `talking` clip is an `.mp4` (no alpha channel possible), rendered directly via `<video>` with no compositing — exposing its recorded backdrop as an opaque black rectangle. `VideoAvatarCanvas` now decodes into a hidden `<video>` and composites every frame onto a visible `<canvas>`; non-webm clips are keyed through `NvaChromakeyGL` (the previously-built but never-wired layered-player chromakey shader) against `manifest.chroma_key` or `background.color`.
4. **SettingsTab avatar hydration**: unlike `App.tsx`'s `syncAvatarConfig`, `SettingsTab` never re-read `avatarProvider`/`nvaModel` on `naia-config-changed`, so its own detail/preview state could diverge from main after login or remote hydration. Added the matching sync effect.
5. **Discord tab wiring**: `NaiaMetaArea`'s 🌐 Channels tab rendered a hardcoded `채널 기능은 현재 안정화 작업 중입니다.` placeholder instead of the already-complete `ChannelsTab` component (loading/not-configured/no-bindings/error/connected states, channel list, message thread — all pre-existing and tested). Wired directly.
6. **Voice onboarding**: `OnboardingWizard` had no voice step at all. Added one after `provider` (login): free Web TTS on/off with a system-voice picker + preview (`speechSynthesis`).

Items 1~6 were verified by focused Vitest and `tsc`. The first pass also claimed, without adequate evidence, that YouTube `ENDED`-only track transition / the BGM play-pause toggle were "already correct" and that Radio DJ used no timer — both claims were wrong or incomplete, corrected below.

7. **BGM false-positive skip (the real "skips before the song ends" bug)**: `BgmPlayer.tsx`'s `beginPlaybackTimeout` watchdog (`PLAYBACK_TIMEOUT_MS = 12_000`) unconditionally forced a track skip if the iframe's `onStateChange` "playing" message hadn't been observed within 12s — but that *message* (not the actual playback) can be lost after an iframe `src` swap on WebView2 (the surrounding code already had a comment acknowledging this handshake-loss risk, for the command-send direction). A genuinely-playing track with a merely-delayed/lost status message got yanked to an unrelated queued track. Fixed two ways: (a) the watchdog now only marks a diagnostic `"timeout"` status — it no longer calls `recoverAfterQueueExhausted`; only the iframe's own `onError` event (real evidence) still triggers recovery; (b) `infoDelivery`'s reported `currentTime` is now also treated as independent proof of playback while status is still `requested`/`loading` (not only as a refresh once already `"playing"`), so a lost `onStateChange` message no longer prevents the true state from being observed at all.
8. **Track-ended agent notification**: on a genuine `ended` event, Shell only updated passive `bgm` context (`naia.pushContext`) and silently consumed its own queue — there was no active signal telling the agent "a track just ended, decide now." Added `emitAiInterferenceEvent({source:"bgm", action:"music_ended", ...})` at the exact `ended` moment, mirroring the existing `"music_changed"` event already fired on track *start* — both route through `ChatArea`'s `onAiInterferenceEvent` → `handleSend`, an existing, tested path that turns into a real agent turn. What the agent then does with that turn (comment vs. call `skill_youtube_bgm` again) is `naia-agent`'s decision, out of this repo.
9. **Real VoxCPM2 onboarding toggle**: superseding item 6's "informational card" scope-down — the user pushed back, so the onboarding voice step now performs the actual `cascade_installation_status` → `start_cascade` → re-confirm lifecycle (same shape as `SettingsTab`'s toggle, deliberately re-implemented rather than refactored out of `SettingsTab` to avoid regression risk in that file) instead of only linking to Settings.

10. **`BgmPlayer.tsx` component test coverage** (was a gap, closed after the user asked for it explicitly): added `components/__tests__/BgmPlayer.test.tsx` — no false skip from the 12s watchdog when there is a queued track and zero corroborating evidence; `infoDelivery` promotes `requested`/`loading` straight to `playing` without ever seeing `onStateChange`, and that promoted status survives the watchdog firing; `music_ended` fires exactly once on a genuine `ended` event and not from the watchdog path.

Verification: Shell production `tsc --noEmit` clean; full Shell Vitest suite green (1463 tests, 0 failed — one earlier failure in an unrelated timing-sensitive local-voice-prebuffer test was confirmed a load-induced flake, not a regression: passed 3/3 in isolation and in two subsequent full-suite re-runs). New/updated focused tests — `lib/avatar/__tests__/media-runtime-routing.contract.test.ts` (rewritten for the new ownership contract), `lib/avatar/__tests__/prebaked-renderer.test.ts` (new), `components/__tests__/NaiaMetaArea.test.tsx` (new), `components/__tests__/SettingsTab.test.tsx` (new hydration regression case), `components/__tests__/ChatArea.test.tsx` (6 cases migrated to the new renderer contract + 1 new case proving dynamic speech routes through Shell synthesis), `components/__tests__/OnboardingWizard{,.newcore}.test.tsx` (updated for the new voice step + new case proving the real `start_cascade` invocation and persisted config), `components/__tests__/BgmPlayer.test.tsx` (new, item 10). `docs/requirements.md` and `docs/user-scenarios.md` got a 2026-08-08 addendum (6 new FR rows, 7 new UC rows) — the WNV issue's originally-sketched `REQ-016~019` naming (section 5 below) was never actually reconciled into those documents; the real, already-active IDs are `FR-NVA-WEB`/`FR-LOCAL-VOICE`/`FR-MEDIA-CONSENT`/`FR-SHELL-UX`/`FR-WIN-DISCORD`.

**Playwright, actually executed this session** (installed a missing Chromium build and ran against a real dev server): `e2e/onboarding-fresh.spec.ts` — 3/3 passed, including the new voice-step click-through. `e2e/bgm-skill.spec.ts` — first run caught a real, valuable regression: an existing test (`재생 관측 시간초과 뒤 준비된 후보를 한 번만 시작한다`) asserted the *old* behavior item 7 deliberately removed (force-promote a queued track purely on the 12s timeout). Rewrote it to assert the new contract (current track and queue untouched, status goes diagnostic `"timeout"`, no extra iframe request) — re-run: 12/12 passed, including the 8-hour logical soak. `e2e-tauri` (native Tauri/WebDriver) suites were not run — that needs a full Rust build and native driver setup this session didn't attempt. NVA chroma-key compositing correctness still has not been visually confirmed on real Windows hardware (Playwright's Linux/headless chromium doesn't exercise the WebView2-specific alpha/codec path this fix targets).

**Still not done**: no dedicated `BgmPlayer.tsx` component test suite exists — items 7/8 are verified by code reading, not an automated test, and are explicitly flagged as a gap in the requirements addendum. Playwright/e2e-tauri suites (NVA, Radio DJ, Discord) were not re-executed this session; `e2e/onboarding-fresh.spec.ts` was updated for the new voice step but not run (needs a live dev server). NVA chroma-key compositing correctness has not been visually confirmed on real Windows hardware.
