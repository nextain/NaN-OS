...
## 나머지 계열까지 실제 증거로 채웠다 (2026-08-26)

앞선 보고에서 "16개는 확인 수단이 없다 — 아직 구현되지 않은 형제 이슈들"이라고 적었는데
**틀렸다**. 네 계열 모두 구현과 계약 테스트가 이미 있었고, 문서 Test Coverage Map 에
매핑도 다 있었다. 비어 있던 것은 내가 손으로 옮겨 적은 벤치 매니페스트였다.

그래서 매니페스트를 손으로 적지 않게 바꿨다. `docs/user-scenarios.md` 의 Test Coverage Map 에서
직접 읽고, 문서가 선언한 파일이 실제로 없으면(문서 부패) 그 자체로 실패시킨다. 손으로 더하는
것은 문서 표에 적히지 않는 실제 환경 증거뿐이다.

그 가드가 곧바로 **진짜 구현 공백 네 개**를 짚었다. 문서가 선언했는데 실제로 없던 검증들이다.

| 만든 것 | 무엇을 |
|---|---|
| `e2e/env-tool-browser.spec.ts` | 브라우저 도구 등록·참조 기반 조작·전후 관측·중단 신호 |
| `e2e/env-tool-injection.spec.ts` | 페이지 본문이 자료 자리로만 오가는지 |
| `e2e-tauri/specs/orchestration-restart.spec.ts` | 재시작 뒤 참조는 남고 실행 상태는 복사되지 않는지 |
| `e2e-tauri/specs/channel-reboot.spec.ts` | 재부팅 뒤 정체성·처리 이력·기밀 경계 |

이어서 남은 등급 부족을 실제 환경으로 채웠다. 살아 있는 Herdr 에 붙는 제어면 어댑터를
처음 만들었고(그 포트에는 여태 실제 구현이 하나도 없었다), 전용 워크스페이스 안에서
관측·변경·멱등·충돌·재접속을 밟았다. 환경 도구는 실제 터미널에서 구조화 실행·취소·권한
경계를 밟았다. 워크스페이스 컨텍스트는 실제 디스크에 임시 워크스페이스를 만들어 해석했다.

### 실제로 붙여 보니 나온 결함들

**뇌에 pane 식별자가 새고 있었다.** 이름이 하나도 없는 pane 은 `surfaceLabel(pane, surfaceId)`
때문에 자기 pane 식별자를 이름으로 올렸다. 손잡이를 불투명하게 만들어 둔 이유가 이름 자리로
새어 나가 무너져 있었다. 대역 스냅샷은 항상 이름이 있어서 안 보였고, 살아 있는 Herdr 에
이름 없는 pane 이 있어서 잡혔다.

**벤치 픽스처가 실제로는 쓸 수 없는 모양을 만들고 있었다.** 진입점 선언의 경로를 백틱 없이
적어 실제 파서가 읽지 못했고(`entrypoint-malformed`), 프로젝트 진입점에는 선언 자체가 없었다.
NFR-AGENT-BENCH.2 가 약속한 "중첩 진입점을 가진 워크스페이스"가 대역으로만 성립하고 있었다.

**계열 단위 증거 요구가 거칠었다.** 환경 도구 계열에 일괄로 브라우저 증거를 요구해서 터미널
실행 시나리오까지 브라우저를 요구했다. 시나리오별 override 를 두어 실제로 무엇을 거쳐야
하는지에 맞췄다.

### 지금 34개 중 30개 (88%)

남은 넷은 `UC-ORCHESTRATION-*` 이고 `worker` 증거를 요구한다 — 실제 코딩 작업자를 띄워야
한다는 뜻이다. 사용자 환경에 프로세스와 자격증명이 걸리는 일이라 이 세션에서 하지 않았고,
루크의 레이어 결정상 오케스트레이션은 naia-agent 몫이기도 하다. 벤치가 이 자리를 계속
미검증으로 보고하는 것이 사실이므로 덮지 않는다.

## 34개 전부 (2026-08-27)

마지막으로 남아 있던 `UC-ORCHESTRATION-*` 넷을 실제로 도는 작업자로 채웠다.

**코딩 모델은 띄우지 않았다.** provider 는 `shell` 이고, 실제 프로세스가 임시 디렉터리 안에서
자기 소유 경로에 산출물을 남긴다. 필요한 증거는 "대역이 아니라 진짜 작업자가 돌았고 산출물이
남았는가"이고 그것은 shell 작업자로 성립한다. 사용자의 자격증명과 비용이 걸리는 일은 이
검증의 목적이 아니다. 어댑터는 `codex`·`claude` 제공자가 오면 시작하지 않고 실패로 답한다 —
띄운 척하지 않는다는 것을 테스트로 고정했다.

이 포트에도 실제 구현이 하나도 없었다. 대역 작업자는 우리가 정한 상태를 그대로 돌려주므로
"작업자가 다 했다고 말해도 판정에 쓰이지 않는가"를 증명하지 못했다. 실제 어댑터는 늘 완료를
주장하고 `production` 권한을 더 달라고 하도록 만들어 두었고, 코어가 그것을 부여하지 않되
거절한 사실은 기록으로 남긴다는 것을 확인했다.

### 마지막에 잡힌 결함

**같은 파일이 낸 두 등급 중 하나가 조용히 사라지고 있었다.** 매니페스트 중복 제거가 파일
경로만 보고 묶어서, 실제 작업자 증거(worker)와 실제 디스크 증거(native)를 같은 파일이 낼 때
뒤엣것이 버려졌다. 그래서 등급이 채워졌는데도 부족하다고 보고됐다. 등급까지 열쇠에 넣어 고쳤다.

### 34개 중 34개

"아직 증명되지 않은 시나리오: 없다."

이것은 이 저장소가 완벽하다는 뜻이 아니라, **선언한 시나리오마다 요구한 등급의 증거가 실제로
모였다**는 뜻이다. 벤치가 더 세게 물어야 할 곳이 있다면 그것은 시나리오와 요구 등급을 다시
정하는 일이지, 지금 통과한 것을 의심하는 일이 아니다.
