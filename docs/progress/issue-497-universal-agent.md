...
## 배선 테스트 감사 (2026-08-26)

"충분한가"를 물어 세어 봤더니 아니었다. 세 곳이 비어 있었다.

**interrupt 가 어디에서도 검증되지 않았다.** focus 와 run 은 계약·도구·실 UI 세 층에서
밟았는데 interrupt 는 0건이었다. 하필 키 표기법이 실제 환경에서 확인되지 않은
(`interruptVerified: false`) 경로다 — 안 밟아 보면 형태조차 모르는 자리였다. 계약 3건과
도구 2건을 더했다. 멈추는 것도 타이핑이므로 터미널 입력 권한을 요구한다는 것,
에이전트가 붙은 표면도 구조화 경로가 없어 키로 간다는 것을 함께 고정했다.

**앱 전환에서 환경 도구가 살아남는지가 주석으로만 단언돼 있었다.** 화면 앱은 전환 시
`app_skills_clear` 대상이다. 환경은 화면 앱이 아니라 상시 표면이라 대상이 아니어야 하는데,
아니라는 근거가 코드 주석뿐이었다. 지워지면 나이아가 첫 앱 전환 이후로 작업 표면을 조용히
못 보게 된다. 실 UI 로 고정했고, 전환이 실제로 일어났는지 먼저 확인한 뒤 단언한다 —
전환이 없으면 그 단언은 아무것도 증명하지 못한다.

**벤치가 이 슬라이스를 통째로 몰랐다.** `agent-bench-scenarios.ts` 는 UC 를 문서에서 읽지만
계열(FAMILIES)에 없는 UC 는 조용히 버린다. #502·#497 의 UC 11개가 그렇게 버려지고 있었다.
문서와 하네스가 어긋나는 걸 막겠다는 파일인데 한쪽 방향만 보고 있었던 셈이다.

계열 다섯을 더하고(ENV-SURFACE·ENV-DISPATCH·ENV-LIVE=native, ENV-STICKY·WIRE-UNION=결정론),
버려지는 방향을 막는 테스트를 넣었다. 이제 계열에도 없고 "에픽 밖" 목록에도 없는 UC 가
생기면 실패한다. 시나리오는 23개에서 34개가 됐다.

기존 단언 둘은 숫자만 올리지 않고 뜻을 다시 세웠다. 개수를 손으로 박아 둔 자리는 문서에서
센 값과 대조하도록 바꿨고("UC 를 더할 때마다 숫자만 올리는" 형태를 없앤다), "결정론 계열은
하나뿐"이라는 단언은 실제 환경을 건드리는 계열이 결정론으로 통과할 수 없다는 성질로 바꿨다.
지켜야 하는 것은 개수가 아니라 무엇이 결정론으로 통과할 수 있느냐다.

검증: 코어 835/835 GREEN(exit 0), tsc 0, 빌드 성공, 도구 15/15, 실 UI 6/6 GREEN(exit 0),
e2e-tauri 16/16 GREEN(exit 0).

남은 것: 벤치는 아직 "무엇을 밟아야 하는가"의 목록이다. 실제로 밟아 판정하는 실행부는
#498 의 native 게이트가 살아나야 돌아간다. 그리고 셸이 낸 값을 뇌가 그대로 받는 것은
공유 표본 양쪽 대조로 닫혀 있지만, 한 프로세스에서 왕복하는 통합 테스트는 없다 —
두 저장소가 갈라져 있어 그 형태를 만들려면 별도 하네스가 필요하다.

## #498 벤치 실행부 (2026-08-26)

시나리오 목록과 판정 규칙은 있는데 실제로 돌려 보는 부분이 비어 있었다. `BenchExecutionPort`
구현이 프로덕션에 하나도 없고 테스트 대역만 있었다 — 그래서 벤치는 "무엇을 확인해야 하는가"의
목록일 뿐 점수를 낸 적이 없다. 그 자리를 채웠다.

원칙 세 가지로 만들었다. 증거는 실제로 돌린 결과에서만 나온다(명령이 실패하면 영수증을
만들지 않는다). 확인 수단이 없는 시나리오는 조용히 넘어가지 않고 증거 없음으로 남는다.
완료 주장은 실행부가 만들지 않고 `docs/requirements.md` 가 Done 이라고 적은 것을 읽는다 —
문서가 완료를 말하고 벤치가 증거를 요구하는 구조라야 거짓 완료 탐지가 의미를 갖는다.

등급은 실행기가 아니라 무엇을 실제로 건드리느냐로 정한다. Playwright 는 실제 DOM 을 그리지만
Tauri IPC 가 대역이라 browser 이고, e2e-tauri 는 실 Rust 백엔드까지 가므로 native 다.
등급을 실제보다 높여 적으면 벤치가 거짓말을 하게 되므로 그것을 막는 테스트를 함께 뒀다.

### 첫 실행이 드러낸 것

**하네스가 주변 환경에 기대고 있었다.** e2e-tauri 단계가 전부 실패했는데, 원인은 제 셸에만
있던 `TAURI_BINARY`·`LD_LIBRARY_PATH` 였다. "내 셸에서는 되는데"가 되면 아무도 재현하지
못한다. 각 단계가 저장소에서 실행 파일과 음성 라이브러리 경로를 직접 찾아 스스로 갖추게 했다.

**매니페스트가 없는 파일을 가리키고 있었다.** 시나리오 id 만 검사하는 가드는 이것을 못 잡았고,
벤치는 "확인 수단이 실패했다"고만 말해 원인이 안 보였다. 모든 단계의 대상 파일이 실제로
존재하는지 보는 가드를 넣었다.

**살아 있는 Herdr 이 대역과 다른 모양을 냈다.** UC-ENV-LIVE-OBSERVE 에 native 증거를 만들려고
실제 Herdr 을 읽기 전용으로 조회하는 테스트를 썼는데, 첫 판본이 실패했다. 실제 세션은
pane 15개 중 5개가 **완전히 같은 이름**이었다 — 뇌는 이름으로 이것들을 구분할 수 없고
손잡이로만 구분된다. 손잡이가 있는 이유가 실제 데이터에서 드러난 셈이고, 대역 스냅샷으로는
영원히 안 보였을 사실이다.

### 지금 상태

시나리오 34개 중 6개 수용. 나머지는 왜 아직인지가 시나리오별로 적힌다.
결과는 `benchmark/agent-bench-report.md`.

돌리는 법: `NAIA_BENCH_LIVE=1 npx vitest run src/test/agent-bench-live.test.ts --testTimeout=1800000`

**조작(UC-ENV-LIVE-ACT)에는 native 증거가 없고, 일부러 만들지 않았다.** 실제로 밟으려면
사용자의 살아 있는 터미널에 명령을 넣어야 해서 전용 Herdr 세션 없이는 할 일이 아니다.
벤치가 이 자리를 계속 "요구 등급의 확인을 더해야 한다"로 보고하는 것이 사실이므로 덮지 않는다.

## 등급 부족을 채웠다 (2026-08-26)

벤치가 "요구 등급의 증거가 빠졌다"고 지목한 자리를 실제 증거로 채웠다.
막고 있던 것은 조작을 실제로 밟으면 사용자의 작업 터미널을 건드린다는 점이었다.
전용 워크스페이스를 만들어 그 안에서만 조작하고 끝나면 닫는 방식으로 풀었다.

안전은 두 겹이다. 테스트가 자기 워크스페이스를 만들고, 명령 포트가 그 워크스페이스
밖의 표면을 거부한다 — 손잡이 해석이 틀려도 남의 터미널로 나갈 수 없다. 실제로 쓰는
명령은 Rust `herdr_run_pane`·`herdr_send_keys` 가 부르는 것과 같은 것이다. 다른 경로를
쓰면 여기서 통과해도 실제 앱과 다른 것을 증명하게 된다.

### 실 환경이 알려 준 것 — 갓 만든 표면은 입력을 삼킨다

첫 실행에서 run 만 실패했다. 명령은 나갔는데 터미널에 아무 일도 일어나지 않았다.
갓 만든 표면은 셸이 다 뜰 때까지 입력을 삼키고, **오류도 내지 않는다**. 프롬프트를
기다린 뒤 보내면 확실히 들어간다.

이건 테스트 사정이 아니라 환경의 성질이다. 나이아가 터미널을 열자마자 명령하면
같은 일이 벌어진다 — 명령이 조용히 사라지고 나이아는 성공했다고 믿는다.
대역 스냅샷으로는 영원히 안 보였을 사실이고, 후속 슬라이스에서 다뤄야 한다.

### 채운 자리

| 시나리오 | 무엇으로 |
|---|---|
| UC-ENV-LIVE-ACT | 전용 워크스페이스에서 실제 실행·중단, 권한 꺼짐 시 무효과까지 |
| UC-ENV-SURFACE-ACT | 번역된 호출이 실제 터미널에서 효과를 낸다 |
| UC-ENV-SURFACE-OBSERVE | 살아 있는 Herdr 이 내는 실제 모양으로 관측 |
| UC-ENV-SURFACE-DENY | 도달 경로가 없는 의도가 실제 환경에서도 거절된다 |
| UC-ENV-SURFACE-DATA | 실제 터미널이 만든 지시문 같은 이름이 자료로만 올라온다 |

`UC-ENV-SURFACE-DATA` 는 실제 pane 이름을 지시문처럼 바꿔 두고 관측했다. 대역 문자열로는
"실제로 그런 이름이 올 수 있나"를 증명하지 못한다.

### 지금 34개 중 14개

등급 부족은 0이 됐다. 남은 20개 중 16개는 확인 수단 자체가 없고(아직 구현되지 않은
형제 이슈들), 4개는 요구사항이 아직 Done 이 아니다.

그 4개는 `UC-WORKSPACE-CONTEXT-*` 이고, 확인은 통과하지만 #501 이 naia-agent 이관
대기 상태라 완료가 아니다. **점수를 올리려고 Done 을 찍지 않았다** — 그러면 벤치를
만든 이유가 없어진다. #498 의 세 시나리오는 이번에 실행부까지 완성돼 실제로 완료라
Done 으로 올렸다.
