...
- [품질 프로세스](./quality-process.md) — 기능이 되는지를 넘어 쓸 만한지를 어떻게 재는가. 층 넷, 품질 축 셋(사용성·안정성·성능), 배포 전 절차, 게이트 목록.
- [회귀 실행 기록](./regression-runs/README.md) — 배포 전 회귀를 여러 기계로 나눠 돌리고 그 결과를 모으는 자리.
- [실기 스펙 요구조건 목록](./e2e-inventory.json) — 실기 e2e 스펙마다 무엇을 요구하는지(자격증명·장치) 기계가 뽑아 둔 목록.
- [저장 위치](./storage-locations.md) — 셸이 무엇을 어디에 저장하는지 실측. 설정·데이터는 ADK, 실행 부산물은 데이터 홈.
