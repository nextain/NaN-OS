...
import { snapshotToReport, surfaceLabel, toRawSurface, type HerdrPaneLike } from "../main/adapters/herdr-environment.js";
...
describe("pane → 원시 표면", () => {
  it("식별할 수 없는 pane 은 뇌에 올리지 않는다", () => {
    expect(toRawSurface({ label: "이름만 있음" })).toBeNull();
    expect(toRawSurface({ pane_id: "", label: "빈 식별자" })).toBeNull();
  });

  it("focused 는 정확히 true 일 때만 참이다", () => {
    expect(toRawSurface(pane({ focused: true }))?.focused).toBe(true);
    expect(toRawSurface(pane({ focused: "true" }))?.focused).toBe(false);
    expect(toRawSurface(pane({ focused: undefined }))?.focused).toBe(false);
  });
});

describe("스냅샷 → 보고", () => {
  it("pane 을 표면으로 싣는다", () => {
    const report = snapshotToReport({ panes: [pane({ pane_id: "a" }), pane({ pane_id: "b" })] });
    expect(report.surfaces.map((s) => s.ref.token)).toEqual(["a", "b"]);
  });

  it("Herdr 가 실제로 내는 세 가지 상태를 정규화한다 — working·idle·unknown", () => {
    const report = snapshotToReport({
      panes: [
  ...
  it("상한을 넘으면 몇 개를 못 실었는지 남긴다", () => {
    ...
    const report = snapshotToReport({ panes });
  ...
  it.each([
    ...
    const report = snapshotToReport(snapshot as never);
...
describe("살아 있는 Herdr", () => {
  ...
  it.skipIf(live === null)("실제 스냅샷에서 표면이 나온다", () => {
    const report = snapshotToReport(live as never);
    expect(report.surfaces.length).toBeGreaterThan(0);
  });

  it.skipIf(live === null)("모든 표면에 이름이 있다 — 레이블 없는 pane 이 섞여 있어도", () => {
    for (const surface of snapshotToReport(live as never).surfaces) {
  ...
  it.skipIf(live === null)("사용자가 보고 있는 표면은 많아야 하나다", () => {
    const focused = snapshotToReport(live as never).surfaces.filter((s) => s.focused);
