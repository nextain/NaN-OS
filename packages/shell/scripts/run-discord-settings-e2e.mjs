...
console.log(`[discord-settings-e2e] WebDriver port ${port}`);
...
const child = spawn(
	...
	[
		...
		"wdio",
		"run",
		"e2e-tauri/wdio.conf.discord-settings.ts",
	],
	{
		cwd: shellRoot,
		stdio: "inherit",
...
child.once("error", (error) => {
	console.error(`[discord-settings-e2e] Failed to start WebDriver: ${error.message}`);
	process.exitCode = 1;
});
child.once("exit", (code, signal) => {
	if (signal) {
		console.error(`[discord-settings-e2e] WebDriver terminated by ${signal}`);
		process.exitCode = 1;
	} else {
