...
if (configuredVitePort !== 1422)
	throw new Error("radio native E2E binary is fixed to Vite port 1422");
export const vitePort = 1422;
export const bgmPort = Number(process.env.NAIA_E2E_BGM_PORT ?? "18772");
export const oauthPort = Number(
	process.env.NAIA_E2E_OAUTH_CALLBACK_PORT ?? "18793",
);
const rootParent =
	process.platform === "win32" ? resolve("D:/tmp") : resolve(tmpdir());
...
async function requireFree(port: number) {
	if (await portOpen(port))
		throw new Error(
			`port ${port} is already occupied; refusing to replace another process`,
		);
}
async function waitForPort(port: number, child: ChildProcess) {
	...
	while (Date.now() < deadline) {
		...
		if (child.exitCode !== null)
			throw new Error(`owned process exited before port ${port} became ready`);
...
export function configure() {
	...
	process.env.NAIA_BGM_SCRIPT = resolve(
		shellDir,
		"..",
		"bgm-sidecar",
		"dist",
...
export function reset() {
	...
	for (const dir of [settings, runtime, webview, appData])
...
export async function start(binary: string) {
	for (const port of [vitePort, e2ePort, bgmPort, oauthPort])
		await requireFree(port);
	vite = spawn(
		execPath,
		[
		...
		],
		{
			...
			env: {
				...
				VITE_NAIA_E2E_ADK_PATH: workspace,
				VITE_NAIA_BGM_BASE: `http://127.0.0.1:${bgmPort}`,
	...
	vite.stderr?.on("data", (data: Buffer) =>
		process.stderr.write(`[radio-queue-e2e:vite] ${data.toString()}`),
	);
	...
	app = spawn(binary, [], {
		...
		env: {
			...appEnvironment(),
			RUST_LOG: "tauri_plugin_wdio_webdriver=debug",
			TAURI_WEBDRIVER_PORT: String(e2ePort),
		},
	});
	app.stderr?.on("data", (data: Buffer) =>
		process.stderr.write(`[radio-queue-e2e:app] ${data.toString()}`),
	);
	const appExit = new Promise<never>((_, reject) =>
		app?.once("exit", (code, signal) =>
			reject(
				new Error(
					`owned Tauri E2E app exited before WebDriver was ready (code=${code}, signal=${signal})`,
				),
			),
		),
	);
