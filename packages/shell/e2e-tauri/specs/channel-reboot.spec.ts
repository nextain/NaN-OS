...
const PROBES: Probe[] = [
	{
		...
		args: {
			...
			json: JSON.stringify({
				...
				channelOutbox: [
					{ channel: "discord", kind: "progress", text: "작업 진행 중" },
				],
			}),
		},
	},
...
describe("채널 재연결 이후 태도 — 영속 경계 (#503) [UC-CHANNEL-SESSION-RECONNECT]", () => {
	...
	it("한 이슈에 대화 정체성이 하나만 남는다 (UC-CHANNEL-SESSION-HANDOFF)", () => {
		...
		for (const id of ids) {
			(
				byIssue.get(id.issue) ?? byIssue.set(id.issue, new Set()).get(id.issue)
			)?.add(id.conversationId);
			(
				bySpace.get(id.issue) ?? bySpace.set(id.issue, new Set()).get(id.issue)
			)?.add(id.spaceId);
		}
		for (const [issue, convs] of byIssue) {
			if (convs.size > 1)
				throw new Error(
					`${issue} 에 대화가 갈라졌다: ${[...convs].join(", ")}`,
				);
		}
		for (const [issue, spaces] of bySpace) {
			if (spaces.size > 1)
				throw new Error(
					`${issue} 에 실행 소유자가 둘이다: ${[...spaces].join(", ")}`,
				);
		}
	...
	it("워크스페이스 기밀이 채널로 나가는 자리에 적히지 않는다 (UC-CHANNEL-SESSION-DISCLOSURE-DENY)", () => {
		...
		if (outbox.length === 0)
			throw new Error("채널 발신함이 디스크에 남지 않았다");
		for (const m of outbox) {
			// 발신함에는 진행 알림만 있고 워크스페이스 경로·기밀 표식이 실리지 않는다.
			if (
				/\/var\/home|\/home\/|workspace-internal|API_KEY|secret/i.test(m.text)
			) {
		...
		expect(
			[...new Set(outbox.map((m) => m.kind))].every(
				(k) => k === "reply" || k === "progress",
			),
		).toBe(true);
	});
