	...
	it("reads Agent's exact store through the real native Shell IPC", async () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				version: 1,
				facts: [
					{ id: "new-boundary", content: "memory/store.json", importance: 1 },
				],
			}),
			{ mode: 0o600 },
		);
		writeFileSync(
			...
			JSON.stringify({
				version: 1,
				facts: [
					{ id: "legacy-boundary", content: ".memory/alpha-memory.json" },
				],
			}),
			{ mode: 0o600 },
		);

		const facts = await tauriInvoke<Array<{ id: string; content: string }>>(
