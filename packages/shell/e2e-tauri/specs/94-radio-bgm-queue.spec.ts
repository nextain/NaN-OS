	...
	return browser.execute(
		async (name: string, input: Record<string, unknown>) => {
			const candidate = window as typeof window & {
				__TAURI_INTERNALS__?: {
					invoke: (n: string, a: unknown) => Promise<unknown>;
				};
				__TAURI__?: {
					core?: { invoke: (n: string, a: unknown) => Promise<unknown> };
				};
			};
			const fn =
		...
		await expect(
			invoke("send_to_agent_command", {
				message: JSON.stringify({
					type: "tool_request",
					requestId: "radio-native-agent-health",
					toolName: "health_probe",
				}),
			}),
		).resolves.toBeNull();
	});
	...
	it("keeps B queued until the active fixture reports ended, then advances to B", async () => {
		...
		try {
			...
			try {
				...
				/* browser log endpoint is optional */
			}
			throw new Error(
				`BGM player did not mount: ${String(error)}; document=${JSON.stringify(documentState)}; browserLogs=${JSON.stringify(browserLogs)}`,
			);
		}
		await invoke("e2e_emit_bgm_event", {
			action: "play",
			videoId: "native-a",
			title: "Native Queue A",
		});
		await browser.waitUntil(
			async () =>
				(await player.getAttribute("data-bgm-current-title")) ===
				"Native Queue A",
			{ timeout: 30_000, timeoutMsg: "A did not mount" },
		);
		await invoke("e2e_emit_bgm_event", {
			action: "enqueue",
			videoId: "native-b",
			title: "Native Queue B",
		});
		expect(await player.getAttribute("data-bgm-current-title")).toBe(
			"Native Queue A",
		);
		...
		await browser.waitUntil(
			async () =>
				(await player.getAttribute("data-bgm-current-title")) ===
				"Native Queue B",
			{ timeout: 30_000, timeoutMsg: "B did not advance after observed end" },
		);
		expect(await player.getAttribute("data-bgm-queue-length")).toBe("0");
		await invoke("e2e_emit_bgm_event", { action: "stop" });
		await browser.waitUntil(
			async () =>
				(await player.getAttribute("data-bgm-playback-status")) === "ended",
			{ timeout: 30_000, timeoutMsg: "stop did not end the active playback" },
		);
		expect(await player.getAttribute("data-bgm-queue-length")).toBe("0");
	});
});
