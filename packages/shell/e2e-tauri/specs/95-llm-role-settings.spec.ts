...
function readPersistedConfig(): Record<string, unknown> {
	if (!adkPath)
		throw new Error("NAIA_E2E_ADK_PATH is required for role settings E2E");
	return JSON.parse(
...
describe("LLM role settings through the real Tauri Shell", () => {
	it("stores separate sub settings, inherits memory from main, and restores both after a WebView restart", async () => {
		...
		expect(await subProvider.$("option[value='codex']").isExisting()).toBe(
			false,
		);
		...
		await browser.waitUntil(
			...
			{
				timeout: 30_000,
				timeoutMsg: "role settings were not written to the isolated ADK config",
			},
		);
		...
		await browser.waitUntil(
					...
					"inherit:main",
			{
				timeout: 30_000,
				timeoutMsg:
					"persisted LLM role settings did not hydrate after WebView restart",
			},
		);
		expect(await (await $("[data-testid='sub-llm-mode']")).getValue()).toBe(
			"explicit",
		);
		expect(await (await $("[data-testid='sub-llm-provider']")).getValue()).toBe(
			"gemini",
		);
