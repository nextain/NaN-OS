...
1. **아바타(Alpha) 표정 및 립싱크 (VRM) 테스트**
   - 질문: "알파야, 오늘 기분 어때? 활짝 웃어줄래?"
...
3. **실제 채널(Discord/Slack) Webhook 발송 (Real-World Test)**
   - 사전 준비: `~/.cafelua/config.json`에 실제 작동하는 Discord 또는 Google Chat Webhook URL 입력.
   - 질문: "내 디스코드로 '안녕? 난 알파야!' 라고 메시지 보내줘."
