...
ls -la shell/src-tauri/target/debug/naia-shell  # Tauri 바이너리 존재 확인

# 전체 실행
cd Naia-OS/shell && pnpm run test:e2e:tauri

# 특정 spec만 디버깅
cd Naia-OS/shell && pnpm exec wdio run e2e-tauri/wdio.conf.ts --spec e2e-tauri/specs/37-execute-command.spec.ts
```
...
## 실행 환경 요구사항
- **Gateway**: `~/.naia/openclaw/node_modules/.bin/openclaw gateway run --bind loopback --port 18789`
- **Tauri 바이너리**: `shell/src-tauri/target/debug/naia-shell`
