# Cafelua OS: 독립 AI 에이전트 OS 비전 리서치
...
- **Cafelua = Lu(ke) + A(lpha)** — 루크와 알파가 함께하는 공간이 곧 OS
...
| **Cafelua OS에 가져올 것** | **Gateway 아키텍처**, 채널 통합, Skills 시스템, 프라이버시 퍼스트 |
...
| **Cafelua OS에 가져올 것** | **Client/Server 분리**, Provider 추상화, LSP, Permission 시스템 |
...
| **Cafelua OS에 가져올 것** | **페르소나 시스템**, SmartEditEngine, 문서 파서, 계정 시스템 |
...
| **Cafelua OS에 가져올 것** | **아바타 렌더링(VRM/Live2D)**, 플러그인 프로토콜, 멀티에이전트 조율 |
...
| **Cafelua OS에 가져올 것** | **스킬 카탈로그 패턴**, 에이전트 팀 오케스트레이션, 메모리 아키텍처 |
...
| **Cafelua OS에 가져올 것** | **하이브리드 메모리 검색**, 세션 영속성, 대시보드 UI 패턴 |

---

## Cafelua OS 합성 아키텍처 (초안)
...
│                                                         │
│  [UI Layer]                                             │
│   ├── Cafelua Space (공간 메타포 UX)  ← cafelua.com    │
│   ├── Alpha Avatar (VRM/Live2D)       ← AIRI           │
...
| `cafelua.com/data/alpha/project소개.md` | Alpha 비전 → **OS의 주인공** |
...
❌ 텍스트 채팅 위주 → ✅ Virtual Avatar (Alpha, VRM/Live2D)
❌ 코딩 도구 느낌 → ✅ 독립 UI (OS 셸)
...
| **BlueBuild 커스텀** | 커스텀 이미지 빌드 파이프라인 존재 → Cafelua OS ISO 제작 가능 |
...
### Phase 0: PoC (2주) — "Bazzite에서 Alpha가 인사하는 순간"

**목표**: Bazzite 부팅 → Alpha 아바타가 화면에 나타나서 대화 가능

```
[Bazzite ISO]
  → 부팅
  → [Cafelua Shell] 자동 시작 (Electron/Tauri)
      ├── Alpha Avatar (VRM, AIRI stage-ui 기반)
      ├── 채팅 패널 (텍스트 + 음성)
...
1. **BlueBuild로 Cafelua OS 이미지 정의**
   ```yaml
   ...
   modules:
     ...
     - type: files
       files:
         - source: cafelua-shell
           destination: /usr/share/cafelua/
     - type: systemd
       system:
         enabled: [cafelua-agent.service]
   ```
...
**추가 기능:**
- **터미널 통합** — Alpha가 시스템 명령 직접 실행 (Careti execute_command)
...
- **서브에이전트** — Alpha가 전문 에이전트를 소환 (코딩, 검색, 분석)
- **Skills 시스템** — 사용자가 스킬 추가 가능 (MoltBot 패턴)
- **MCP 서버** — 외부 도구 연결 확장
- **Cafelua Space UX** — 공간 메타포 (라운지/아틀리에) 도입

### Phase 3: 배포 (12주) — "누구나 설치할 수 있는 AI OS"

- **Cafelua OS ISO** 공개 배포 (BlueBuild CI/CD)
- **온보딩 위자드** — 첫 부팅 시 Alpha가 설정 안내
- **API 키 설정** — 사용자 본인 키 or Cafelua 프록시
- **커뮤니티** — 스킬/아바타 마켓플레이스
...
- `/home/luke/dev/cafelua.com/` (Cafelua 개인 홈페이지)
